...
Main script for K-fold cross validation with Logarithmic compression frontend
"""
...
for i in range(10):
    ...
    opt = optim.Adam(model_cnn.parameters(), lr=lr, weight_decay=1e-3)
    ...
    for e in range(0, epochs):
        ...
        for (x, y) in trainLoader:
            ...
            apred = pred.cpu().detach().numpy() # Predictions for checking for identical values
            predicts = [p for p in apred] # List of Predictions
            # Confirm that not all predictions are identical
            ...
            if stop_pred == 50:
                ...
                stop() # Quit training due to cumulative identical predictions
    ...
    results_df.to_csv('../results/experiments/logmel_all/result_data/logmel_'+val_fold)
    ...
    ax[0].set_title('Training/Validation Loss: Log Model')
    ...
    ax[1].set_title('Training/Validation Accuracy: Log Mel')
    ax[1].set_xlabel('Epoch')
    ax[1].set_ylabel('acc')
    ax[1].legend(loc='lower left')

    plt.savefig('../results/experiments/logmel_all/plots/logmel_'+str(rd_fold[i])+'.png')
