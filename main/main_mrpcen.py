...
Main script for K-fold cross validation with Multi-rate PCEN frontend
"""
...
for i in range(10):
    ...
    model_frontend = MRPCEN(n_bands=128, t_values=t_vals,
                            alpha=0.8, delta=10.,
                            r=0.25, eps=10e-6)
    ...
    results_df.to_csv('../results/experiments/mrpcen_all/result_data/mrpcen_'+val_fold)
    # Figures
    ...
    ax[0].set_title('Training/Validation Loss: MRPCEN')
    ...
    ax[1].set_title('Training/Validation Accuracy: MRPCEN')
    ax[1].set_xlabel('Epoch')
    ax[1].set_ylabel('acc')
    ax[1].legend(loc='lower left')

    plt.savefig('../results/experiments/mrpcen_all/plots/mrpcen_'+str(rd_fold[i])+'.png')
