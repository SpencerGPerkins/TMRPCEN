...
Main script for K-fold cross validation on TMRPCEN frontend
"""
...
from birdhouse_l3_multi import birdHouseL3
...
for i in range(10):
    ...
    # Assign model (frontend, cnn)
    model_frontend = TMRPCEN(n_bands=128,
                            alpha=0.8, delta=10.,
                            r=0.25, eps=10e-6)
    ...
    results_df.to_csv('../results/experiments/mrpcents_all/result_data4/mrpcents_'+val_fold)
    ...
    ax[0].set_title('Training/Validation Loss: MRPCEN Trainable S')
    ...
    ax[1].set_title('Training/Validation Accuracy: MRPCEN Trainable S')
    ax[1].set_xlabel('Epoch')
    ax[1].set_ylabel('acc')
    ax[1].legend(loc='lower left')

    plt.savefig('../results/experiments/mrpcents_all/plots4/mrpcents_'+str(rd_fold[i])+'.png')
